...
# ---------- Эндпоинт /quality-from-csv: реальный CSV через нашу EDA-логику ----------
...
async def quality_flags_from_csv(file: UploadFile = File(...)):
    ...
    Эндпоинт, который принимает CSV-файл, запускает EDA-ядро
    (summarize_dataset + missing_table + compute_quality_flags)
    и возвращает полный набор флагов качества по CSV-файлу.
    """
    ...
    quality_flag_response = {
        "flags": flags_all
    }
    return quality_flag_response